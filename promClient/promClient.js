    ...
    testQuery(){
        ...
        ipcMain.on('load:cpu-usage', async () => {
            try {
                ...
                // const rawres = await fetch('http://localhost:9090/api/v1/query?query=100-(avg%20by%20(node)%20(rate(container_cpu_usage_seconds_total%7Bnode=%22docker-desktop%22%7D[1m]))%20*%20100)');
                // console.log(rawres, 'rawres in testQuery');
                // const res = await rawres.json();
                // console.log(res, 'res in testQuery');
                // const CpuUsage = await res.data.result[0].value[1];
                // loadPromClientData(dispatch({type:"load:cpu-usage"}));
                this.window.webContents.send('get:cpu-usage', 'hello');
            }
