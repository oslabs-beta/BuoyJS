...
const NodesCard = (props) => {
  ...
  return (
    ...
    <div className="nsCardFrontBox">
      ...
      <span className="nsFrontName">{ props.name }</span>
      <span className="nsFrontStatus">{props.pods.length}</span>
    </div>
    ...
    <div className="nsCardBackButtonCx"><button className="nsCardBackMaximize" 
      ...
      <div id={`${props.name}`}className='nsAnimation'></div>
      <ul>
        <li><span className="nsBackAttribute">Name: </span><span className="nsBackValue">{props.name}</span></li>
        <li><span className="nsBackAttribute">Pods: </span><span className="nsBackValue">{props.pods.length}</span></li>
      </ul>
      </div>
    </div>
    </React.Fragment>
